            ...
            return (
              <article key={video.id} className={`group/card overflow-hidden rounded-2xl border bg-black/50 transition ${isActive ? 'border-fuchsia-400 shadow-[0_0_24px_rgba(217,70,239,0.22)]' : 'border-white/10 hover:border-white/20'}`}>
                <div className="relative aspect-[9/16] overflow-hidden bg-zinc-950">
                  ...
                  <div className={`pointer-events-none absolute inset-x-0 bottom-0 z-20 h-36 bg-gradient-to-t from-black/40 to-transparent transition-opacity duration-300 ${isActive ? 'opacity-0' : 'opacity-0 group-hover/card:opacity-100 group-focus-within/card:opacity-100'}`} />

                  <span className={`pointer-events-none absolute left-2.5 top-2.5 z-20 inline-flex items-center gap-1 rounded-full border border-white/80 bg-white/85 px-2 py-1 text-[9px] font-black text-emerald-700 opacity-0 shadow-md backdrop-blur-sm transition-opacity duration-300 ${isActive ? 'opacity-0' : 'group-hover/card:opacity-100 group-focus-within/card:opacity-100'}`}>
                    <Check className="h-2.5 w-2.5" /> VIDEO READY
                  </span>

                  <span className={`pointer-events-none absolute left-1/2 top-1/2 z-20 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/80 bg-white/90 text-zinc-900 opacity-0 shadow-xl backdrop-blur-sm transition-all duration-300 ${isActive ? 'opacity-0' : 'group-hover/card:scale-105 group-hover/card:opacity-100 group-focus-within/card:opacity-100'}`}>
                  ...
                  <a href="#viral-studio" tabIndex={isActive ? -1 : 0} className={`flex w-full items-center justify-center gap-1.5 rounded-xl border border-white/80 bg-white/88 px-3 py-2 text-[11px] font-black text-fuchsia-700 shadow-md backdrop-blur-sm transition hover:bg-white ${isActive ? 'pointer-events-none' : 'pointer-events-auto'}`}>
